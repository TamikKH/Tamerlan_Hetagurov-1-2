...
tuple-level locks | Высокая параллельность,  | Возможность deadlock при цикле   | 
                  | блокируется только строка|                                  |
...
Cursor buffer pin | Быстрое последовательное | может задерживать VACUUM FREEZE|
                  | чтение                   | 
