...
Реcovery запускается  | Остановка в режиме immediate, 	        | спользовать -m fast или -m smart
после рестарта	       | отсутствие чекпоинта                   |

Грязные страницы      | Низкая активность checkpointer	        | Выполнять CHECKPOINT;
долго висят в кеше	   |                                        | вручную при необходимости

Таблица пропадает из  | Буферный кеш не сохраняется	           | Использовать pg_prewarm после запуска
кеша после рестарта	  |

Большой размер WAL	   | Включён full_page_writes	              |Возможное снижение — отключение (если допустимо), либо сжатие WAL
WAL почти не          | Страницы уже хорошо сжимаются          | Использовать другие методы 
уменьшается при сжатии| или мало меняются	                     | уменьшения WAL (batching, уменьшение UPDATE)

Незавершённые         | Аварийное завершение сервера	          | Явно фиксировать важные транзакции
транзакции пропадают	 |
